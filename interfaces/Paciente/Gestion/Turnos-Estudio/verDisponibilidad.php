...
session_start();
// Aquí asumo que el paciente ya está logueado y su id está en la sesión
...
// Función para obtener días con disponibilidad en los próximos 15 días
function getDisponibilidadDias($conn, $estudioId, $sedeId) {
    ...
    for ($i = 0; $i < 15; $i++) {
        ...
        // Verifico si hay agenda disponible para ese estudio, sede y fecha
        $sql = "
            SELECT COUNT(*) as cnt 
            FROM agenda a 
...
<head>
    ...
    <style>
        .disponible { color: green; cursor: pointer; }
        .no-disponible { color: red; }
        .dia { margin: 5px; padding: 5px; display: inline-block; border: 1px solid #ccc; }
...
document.querySelectorAll('.disponible').forEach(diaDiv => {
    diaDiv.addEventListener('click', () => {
        ...
        .then(response => response.text()) // usamos text() para debug
        .then(data => {
